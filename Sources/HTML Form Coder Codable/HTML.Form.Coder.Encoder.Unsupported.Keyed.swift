...
extension HTML.Element.Form.Coder.Encoder.Unsupported {
    struct Keyed<Key: CodingKey>: KeyedEncodingContainerProtocol {
        ...
        mutating func encodeNil(forKey key: Key) throws(HTML.Element.Form.Coder.Encoder.Error) {
        ...
        mutating func encode<T>(
            _ value: T,
            forKey key: Key
        ) throws(HTML.Element.Form.Coder.Encoder.Error)
        where T: Encodable {
            throw HTML.Element.Form.Coder.Encoder.Error.encodingError(
                "superEncoder() is not supported in URL form encoding",
                self.codingPath
        ...
        mutating func superEncoder() -> any Swift.Encoder {
            return HTML.Element.Form.Coder.Encoder.Unsupported(codingPath: self.codingPath)
        }

        // reason: stdlib Codable protocol requirement forces this existential (any CodingKey / Encoder / Decoder / *Container); the conforming type cannot narrow it.
        // swiftlint:disable:next no_any_protocol_existential
        mutating func superEncoder(forKey key: Key) -> any Swift.Encoder {
            return HTML.Element.Form.Coder.Encoder.Unsupported(codingPath: self.codingPath + [key])
        }
    }
}
