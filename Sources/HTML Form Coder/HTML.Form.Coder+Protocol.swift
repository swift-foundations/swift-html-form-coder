...
extension HTML.Element.Form.Coder: RFC_9110.Body.Coder.`Protocol` {
    ...
    @inlinable
    public func parse(_ input: inout [Byte]) -> HTML.Element.Form.Data.Entry.List {
        let source = String(decoding: input.map(\.underlying), as: UTF8.self)
        let entries = WHATWG_Form_URL_Encoded.parse(source).map {
            HTML.Element.Form.Data.Entry(name: $0.0, stringValue: $0.1)
        }
        input = []
        return HTML.Element.Form.Data.Entry.List(entries: entries)
    }
