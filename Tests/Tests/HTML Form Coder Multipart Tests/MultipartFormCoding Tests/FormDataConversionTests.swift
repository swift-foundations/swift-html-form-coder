...
struct FormDataConversionTests {
    ...
    func `HTML.Form.Data.Entry.List to Multipart conversion with file`() throws {
        ...
        let hasAvatarPart = multipart.parts.contains { part in
            ...
            return disposition.name == "avatar"
                && disposition.filename?.value == "photo.jpg"
        }
    ...
    func `Content-Type header generation`() throws {
        ...
        #expect(contentType.parameters["boundary"]?.isEmpty == false)
    ...
    func `Custom boundary is preserved`() throws {
        ...
        // Act
        let multipart = try formData.multipart(boundary: customBoundary)
